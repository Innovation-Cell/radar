...
Speed_direction=0
Yaw_rate=0
Yaw_Rate_validation=0 
Steering_angle_rate_sign=0 
Radius_curvature=0
Steering_angle_validation=0
...
class RadarDataParser():
    ...
    def parseMessage(self, msgId, rawmsg, dlc, flg, time):
        ...
        if msgId in msgToFunc:
            ...
            #we pass the data at the last message id =1512
            else:
                if self.debug == True:
    ...
    def track_msg(self, msgId, msg):
       ...
       # print(type((msg[0] & 0x01)))
        ...
        val=(((msg[1] & 0x1F) << 5) | ((msg[2] & 0xF8) >> 3))  # if sign bit is set e.g., 8bit: 128-255
        if (val & (1 << (10 - 1))) != 0: 
            val = val - (1 << 10) 
        self.data[track_id + "_track_angle"] = 0.1*val # Spans multiple bytes
        self.data[track_id + "_track_range"] = 0.1*(((msg[2] & 0x07) << 8) | msg[3]) # Spans multiple bytes
        ...
        #print(self.data[track_id + "_track_width"])
        ...
        vel=(((msg[6] & 0x3F) << 8) | msg[7]) # if sign bit is set e.g., 8bit: 128-255
        if (vel & (1 << (14 - 1))) != 0: 
            vel = vel - (1 << 14) 
        vel1=(18.0/5.0)*0.01*vel
        if vel1<=0.5 and vel1 >=-0.5:
            vel1=0
        self.data[track_id + "_track_range_rate"] = vel1
...
def createMsg():
    ...
    global Steering_angle_rate_sign
    global Yaw_Rate_validation
    global Steering_angle_validation
    global Steering_angle_sign
    Speed_direction = 0 # 0 - forward and 1 - Reverse()
    Yaw_Rate_validation = 1  # 0 - invalid and 1 - valid
    Steering_angle_rate_sign = 0  # 0 - left turn and 1 - right turn 
    Steering_angle_validation = 1 # 0 - Invalid and 1 - valid
    Steering_angle_sign = 0  # 0 - counterclockwise and 1 - clock wise
    msg = [0,0,0,0,0,0,0,0]
    msg[0] = int(format(Vehicle_speed, '011b')[:8], 2)
    msg[1] = int(format(Vehicle_speed, '011b')[8:11] + str(Speed_direction) + format(Yaw_rate, '012b')[:4], 2)
    msg[2] = int(format(Yaw_rate, '012b')[4:], 2)
    msg[3] = int(str(Yaw_Rate_validation) +  str(Steering_angle_rate_sign) + format(Radius_curvature, '014b')[:6], 2)
    msg[4] = int(format(Radius_curvature, '014b')[6:], 2)
    msg[5] = int(str(Steering_angle_validation) + str(Steering_angle_sign) + format(int(Steering_angle), '011b')[:6], 2)
    msg[6] = int(format(int(Steering_angle), '011b')[6:] + format(Steering_angle_rate, '011b')[:3], 2)
    msg[7] = int(format(Steering_angle_rate, '011b')[3:], 2)
...
def SterVelcallback(msg1):   
    ...
    global Steering_angle_sign
    Steering_angle = abs(msg1.theta) ## conversion required to m/s and divide by scaling factor
    Vehicle_speed = int(msg1.x)
...
def Radius():
    ...
    if Steering_angle==0:
        ...
        Radius_curvature = 2/sin(math.radians(Steering_angle)) ## conversion required to m and divide by scaling factor
    #Radius of curvature positive = clockwise         


def YawRateCallback(yaw_r):
    global Yaw_rate
    Yaw_rate = int(int(yaw_r.angular_velocity.z) / 0.0625)  # conversion required to degree/s and divide by scaling factor
    #Yaw rate positive positive = clockwise


def SA_Rate_Callback(Steer_angle_r):
    global Steering_angle_rate
    Steering_angle_rate = 0 ##conversion required to degree/s and divide by scaling factor(set as default value here)  
...
if __name__ == "__main__":
    ...
    pub = rospy.Publisher('esr_front', std_msgs.msg.String, queue_size=1)

    rate = rospy.Rate(10)
    
    rospy.Subscriber('/chatter_vel_sa', Pose2D, SterVelcallback)  ##Vehicle speed 
    rospy.Subscriber('/imu/data', Vector3, YawRateCallback)  ##yaw rate of the vehicle
    #count=0
    while not rospy.is_shutdown():
        
        message1264=createMsg()  
        #print message1264
        ch1.write(1264, message1264, 8) #sending car data to radar
        tm.sleep(.0005)
